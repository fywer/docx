...
class MetadatoRepository(IDocumentoData):
    ...
    def doDelete(self, nombre):
        try:
            comprobanteT = self.__getEntity().comprobantes.find_one_and_delete({"nombre" :nombre})
            log.info(f"Se han eliminado en Atlas: {nombre}")
            return comprobanteT
        except Exception as e:
            raise Exception("DATA: {0}".format(e))
    
    def doUpdate(self, nombre, comprobante):
        try:
            c = self.__getEntity().comprobantes.find_one_and_update({'nombre' : comprobante['nombre']},{ '$set' : comprobante})
            if c is None:
    ...
    def doSelect(self):
        try:
            comprobantes = self.__getEntity().comprobantes.find()
            return comprobantes
        except Exception as e:
            raise Exception("DATA: {0}".format(e))  